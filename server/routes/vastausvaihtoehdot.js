...
router.post('/', async (req, res) => {
    try {
        ...
        const text = 'INSERT INTO kysymys (kysymys_id, nimi, onkooikein) VALUES ($1, $2, $3) RETURNING id'
        const result = await db.query(text, [kysymys_id, nimi, onkooikein])
        res.status(200).send(`Uusi vastausvaihtoehto lisätty kysymykseen: ${kysymys_id}. Vastausvaihtoehdon id on ${result.rows[0].id}`)
    } catch (error) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const text = 'UPDATE vastausvaihtoehto SET kysymys_id = ($1), nimi = ($2), onkooikein = ($3) WHERE id = ($4)'
        console.log(req.body)
        await db.query(text, [uusiKysymys_id, uusiNimi, uusiOikein, req.params.id])
        res.status(201).send(`Vastausvaihtoehto muutettu ID:llä ${req.params.id}. Kysymyksen ID: ${uusiKysymys_id}`)   
    } catch (error) {
        res.status(500).send(error)
...
router.delete('/:id'), async (req, res) => {
    try {
        ...
        res.status(204).send(`Vastausvaihtoehto ID:llä ${req.params.id} poistettu`)
    } catch (error) {
        res.status(500).send(error)
